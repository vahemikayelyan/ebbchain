    ...
    <div class="w-full max-w-md bg-white rounded-xl shadow-md p-8">
      ...
      <div class="space-y-6 mt-3">
        ...
        <h1 class="text-center text-2xl font-semibold text-gray-900">
          {{
            isLogin ? "Sign in to CarroCash" : "Join CarroCash to start earning"
          }}
        </h1>
        ...
        <form @submit.prevent="handleSubmit" class="space-y-4">
          ...
          <!-- API error -->
          <p v-if="errors.api" class="text-xs text-red-600">{{ errors.api }}</p>
          ...
          <button type="submit" class="w-full btn-primary">
            ...
            <span v-else>{{ isLogin ? "Sign In" : "Join Now" }}</span>
          </button>
        </form>
        ...
        <p class="text-center text-sm text-gray-600 font-semibold">
          <span v-if="isLogin">
            ...
            <button
              @click="isLogin = false"
              class="text-carrot-600 hover:underline"
            >
              Join now
            </button>
          </span>
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (isLogin.value) {
      ...
      if (!res?.success) {
        errors.api = res?.message || "Registration failed.";
        return;
      }
      ...
      if (result?.error) {
        errors.api = "Registered, but could not log in automatically.";
        return;
      }
